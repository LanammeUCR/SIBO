...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        $('#tblUnidad thead tr#filterrow th').each(function () {
            var campoBusqueda = $('#tblUnidad thead th').eq($(this).index()).text();
            $(this).html('<input type="text" style="text-align: center" onclick="stopPropagation(event);" placeholder="Buscar ' + campoBusqueda + '" />');
        });

        // DataTable
        var table = $('#tblUnidad').DataTable({
            orderCellsTop: true,
            "iDisplayLength": 10,
        ...
        // aplicar filtro
        $("#tblUnidad thead input").on('keyup change', function () {
            table
